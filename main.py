...
=====================================================
Description:
    Executes baselines and the self-supervised Anomal-E model,
    mapping all metrics into a unified binary evaluation format.
"""
...
def main():
    ...
    # Identify the index for normal traffic vs attack traffic
    normal_label_idx = encoder.transform(['Normal'])[0] if 'Normal' in encoder.classes_ else 0
    
    # Convert true targets into unified binary: 0 for Normal, 1 for any attack/anomaly
    ...
    dl_models = {
        "CNN": CNNModel(X_train.shape[1], 2).to(device),  # 2 output classes for binary detection
    ...
    print("  --> Starting Self-Supervised DGI Mutual Information maximization training...")
    dgi_model.train()
    for epoch in range(15):
        ...
        if dgi_model.training:
            noise = torch.randn_like(salted_feats) * 0.02
            salted_feats += noise
    ...
    print("  --> Serializing GNN components and setting up downstream IsolationForest...")
    encoder_net.eval()
    with torch.no_grad():
        train_embeddings = encoder_net(edge_index, edge_feats).cpu().numpy()[train_mask]
        
    contamination_ratio = max(0.01, min(0.5, np.sum(y_train != normal_label_idx) / len(y_train)))
    
    downstream_detector = get_anomal_e_detector(contamination=contamination_ratio)
    ...
    print("  --> Re-loading unified structures from disk to perform validation evaluation...")
    ...
    # Isolation forest outputs: 1 for normal, -1 for anomaly. Map to 0 and 1.
